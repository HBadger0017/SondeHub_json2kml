...
#####################################################################################
# This python script (v3) will pull the json file for a given radiosonde from sondehub.org
# and convert it to kml for plotting in Google Earth or which ever geo-type application
# used.
#
# ** UPDATE for v3-4 - updated info box output for Google Earth; added local time conversion
...
for list in range(0,len(data),50):
    try:
        ...
        freq = round(data[list]['frequency'], 3) # tx freq of the sonde
        ...
        utc_datetime = datetime.fromisoformat(dt)

        local_datetime = utc_datetime.astimezone(tz=None)

        sonde_time = datetime.strftime(parser.parse(dt),'%Y-%m-%d %H:%M:%S %Z')
        local_time = datetime.strftime(parser.parse(str(local_datetime)), '%Y-%m-%d %H:%M:%S')
        local_tz = local_datetime.tzname()
    ...
    #pnt.description = "<b>{0}</b><br/><br/><u>Make: </u>{10}<br/><u>Model:</u> {11}<br/>Sonde Time:<br/>{1}<br/>TX freq: {2} MHz<br/><br/>Altitude: {6} m<br/>Heading: {7}&deg<br/>Horizontal Velocity: {8} m/s<br/>Vertical Velocity: {9} m/s<br/><br/><b>WX Metrics</b><br/>Tempurature: {3}&degC <br/>Humidity: {4}% <br/>Battery: {5} VDC<br/>".format(title,sonde_time,freq,temp,hum,batt,alt,head,vel_h,vel_v,man,subtype)
    #pnt.style = style
    pnt.description = "<u>RadioSonde:</u><br/><b>{0}</b><br/><br/><u>Make: </u>{12}<br/><u>Model:</u> {13}<br/><u>TX freq:</u> {4} MHz<br/><br/><b>UTC Time:</b><br/>{1}<br/><b>Local Time:</b><br/>{2} {3}<br/><br/><b>Flight Dynamics</b><br/>Altitude: {8} m<br/>Heading: {9}&deg<br/>Horizontal Velocity: {10} m/s<br/>Vertical Velocity: {11} m/s<br/><br/><b>WX Metrics</b><br/>Tempurature: {5}&degC <br/>Humidity: {6}% <br/>Battery: {7} VDC<br/>".format(title,sonde_time,local_time,local_tz,freq,temp,hum,batt,alt,head,vel_h,vel_v,man,subtype)
    pnt.style = style
